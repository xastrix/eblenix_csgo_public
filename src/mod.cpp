...
void mod::init(void* I)
{
	auto h = CreateThread(0, 0, [](void* I) -> unsigned long __stdcall
	{
		...
		g::init_state(CS_CHECK_GAME_VERSION, []() {
		...
		g::init_state(CS_INIT_HOOKS, []() {
			g_input.init({ CSGO_CLASS_NAME, 0 });
			g_hooks.init();
			g_event.init();
		});

		g::init_state(CS_WAITING_FOR_SHUTDOWN, []() {
			...
			while (!g::is_state(CS_SHUTDOWN))
				std::this_thread::sleep_for(std::chrono::seconds(2));
		}, false);

		g::init_state(CS_SHUTDOWN, [I]() {
