...
void ui::draw(int x, int y)
{
	...
	for (int i = 0; i < m_entry_sz; i++)
	{
		...
		g_font.draw_string("+", menu_text_x + head_box_width - 20, menu_text_y - 11, g_font[Tahoma12px],
	...
	if (s_opened[UI_SUB_POS])
	{
		...
		for (int i = 0; i < s_entry_sz[UI_SUB_POS]; i++)
		{
			...
			g_font.draw_stringW(s_entries[i][UI_SUB_POS].m_name, sub_menu_text_x + 3, sub_menu_text_y - 11,
				g_font[Tahoma12px], TEXT_OUTLINE, m_colors[UI_TEXT_COL]);

			if (s_entries[i][UI_SUB_POS].m_state == UI_TAB_STATE)
			{
				g_font.draw_string("+", sub_menu_text_x + head_box_width - 20, sub_menu_text_y - 11, g_font[Tahoma12px],
			...
			else
			{
				if (s_entries[i][UI_SUB_POS].m_state != UI_FUNCTION_STATE)
				{
					...
					{
						draw_bool(g_vars.get_as<bool>(s_entries[i][UI_SUB_POS].m_var).value(),
					...
					{
						draw_int(g_vars.get_as<int>(s_entries[i][UI_SUB_POS].m_var).value(),
							sub_menu_text_x + head_box_width, sub_menu_text_y, m_colors[UI_TEXT_COL]);
					}
					else if (s_entries[i][UI_SUB_POS].m_state == UI_FLOAT_STATE)
					{
						draw_float(g_vars.get_as<float>(s_entries[i][UI_SUB_POS].m_var).value(),
							sub_menu_text_x + head_box_width, sub_menu_text_y, m_colors[UI_TEXT_COL]);
					}
					else if (s_entries[i][UI_SUB_POS].m_state == UI_KEY_STATE)
					{
						draw_hotkey(g_vars.get_as<int>(s_entries[i][UI_SUB_POS].m_var).value(),
					...
					{
						draw_item(s_entries[i][UI_SUB_POS].m_items[g_vars.get_as<int>(s_entries[i][UI_SUB_POS].m_var).value()],
							sub_menu_text_x + head_box_width, sub_menu_text_y, m_colors[UI_TEXT_COL]);
					}
				}
			}
	...
	if (s_opened[UI_SUB_SUB_POS])
	{
		...
		for (int i = 0; i < s_entry_sz[UI_SUB_SUB_POS]; i++)
		{
			...
			{
				g_font.draw_string("+", sub_sub_menu_text_x + head_box_width - 20, sub_sub_menu_text_y - 11, g_font[Tahoma12px],
			...
			else
			{
				if (s_entries[i][UI_SUB_SUB_POS].m_state != UI_FUNCTION_STATE)
				{
					...
					{
						draw_bool(g_vars.get_as<bool>(s_entries[i][UI_SUB_SUB_POS].m_var).value(),
					...
					{
						draw_int(g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_POS].m_var).value(),
					...
					{
						draw_float(g_vars.get_as<float>(s_entries[i][UI_SUB_SUB_POS].m_var).value(),
							sub_sub_menu_text_x + head_box_width, sub_sub_menu_text_y, m_colors[UI_TEXT_COL]);
					}
					else if (s_entries[i][UI_SUB_SUB_POS].m_state == UI_KEY_STATE)
					{
						draw_hotkey(g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_POS].m_var).value(),
					...
					{
						draw_item(s_entries[i][UI_SUB_SUB_POS].m_items[g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_POS].m_var).value()],
							sub_sub_menu_text_x + head_box_width, sub_sub_menu_text_y, m_colors[UI_TEXT_COL]);
					}
				}
			}
	...
	if (s_opened[UI_SUB_SUB_SUB_POS])
	{
		...
		for (int i = 0; i < s_entry_sz[UI_SUB_SUB_SUB_POS]; i++)
		{
			...
			if (s_entries[i][UI_SUB_SUB_SUB_POS].m_state != UI_FUNCTION_STATE)
			{
				...
				{
					draw_bool(g_vars.get_as<bool>(s_entries[i][UI_SUB_SUB_SUB_POS].m_var).value(),
				...
				{
					draw_int(g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_SUB_POS].m_var).value(),
				...
				{
					draw_float(g_vars.get_as<float>(s_entries[i][UI_SUB_SUB_SUB_POS].m_var).value(),
						sub_sub_sub_menu_text_x + head_box_width, sub_sub_sub_menu_text_y, m_colors[UI_TEXT_COL]);
				}
				else if (s_entries[i][UI_SUB_SUB_SUB_POS].m_state == UI_KEY_STATE)
				{
					draw_hotkey(g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_SUB_POS].m_var).value(),
				...
				{
					draw_item(s_entries[i][UI_SUB_SUB_SUB_POS].m_items[g_vars.get_as<int>(s_entries[i][UI_SUB_SUB_SUB_POS].m_var).value()],
						sub_sub_sub_menu_text_x + head_box_width, sub_sub_sub_menu_text_y, m_colors[UI_TEXT_COL]);
				}
			}
