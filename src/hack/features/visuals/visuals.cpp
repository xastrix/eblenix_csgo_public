		...
		if (g_csgo.m_local->is_life_state())
		{
			if (g_csgo.m_local->is_scoped())
			{
				...
				if (weapon && g_helpers.is_sniper(weapon))
				{
					...
					g_render.draw_line(0, Y, g.m_screen_width, Y, 1.0f, color_t(0, 0, 0, 155));
					g_render.draw_line(X, 0, X, g.m_screen_height, 1.0f, color_t(0, 0, 0, 155));
				}
			}
		}
	}
...
void visuals::draw_status(const char* title, int x_offset)
{
	...
	g_render.draw_filled_rect(g.m_screen_width - string_width - 15, 10, g.m_screen_width, 17, ui_shadow_col);

	g_render.draw_filled_rect(g.m_screen_width - string_width - 12, 13, 2, 11,
		g.m_initialized ? color_t("ui->col") : color_t(164, 164, 164));

	g_render.draw_string(title, g.m_screen_width - string_width - 5, 12, g_render.get_font(Tahoma12px), TEXT_OUTLINE, color_t(255, 255, 255));

	if (g_csgo.m_engine->is_connected())
	{
		g_render.draw_filled_rect(g.m_screen_width - string_width - 37, 10, 20, 17, ui_shadow_col);
		g_render.draw_filled_rect(g.m_screen_width - string_width - 59, 10, 20, 17, ui_shadow_col);
		g_render.draw_filled_rect(g.m_screen_width - string_width - 81, 10, 20, 17, ui_shadow_col);

		g_render.draw_filled_rect(g.m_screen_width - string_width - 34, 13, 2, 11,
			g_vars.get_as<bool>("esp->enabled").value() ? color_t("ui->col") : color_t(164, 164, 164));

		g_render.draw_filled_rect(g.m_screen_width - string_width - 56, 13, 2, 11,
			g_vars.get_as<bool>("triggerbot->enabled").value() ? color_t("ui->col") : color_t(164, 164, 164));

		g_render.draw_filled_rect(g.m_screen_width - string_width - 78, 13, 2, 11,
			g_vars.get_as<bool>("aimbot->enabled").value() ? color_t("ui->col") : color_t(164, 164, 164));

		g_render.draw_string("E", g.m_screen_width - string_width - 30, 12,
			g_render.get_font(Verdana12px), TEXT_OUTLINE, color_t(255, 255, 255));

		g_render.draw_string("T", g.m_screen_width - string_width - 52, 12,
			g_render.get_font(Verdana12px), TEXT_OUTLINE, color_t(255, 255, 255));

		g_render.draw_string("A", g.m_screen_width - string_width - 74, 12,
			g_render.get_font(Verdana12px), TEXT_OUTLINE, color_t(255, 255, 255));
	}
}
...
void visuals::draw_spectators()
{
	...
	g_render.draw_filled_rect(g.m_screen_width - 105, Y, g.m_screen_width, 18,
		color_t(0, 0, 0, g_vars.get_as<int>("ui->col->a").value()));

	g_render.draw_filled_rect(g.m_screen_width - 102, Y + 3, 2, 12, color_t("ui->col"));

	g_render.draw_string("Spectators", g.m_screen_width - 96, Y + 2,
	...
	for (int i = 1; i <= g_csgo.m_globals->max_clients; i++)
	{
		...
		g_render.draw_filled_rect(g.m_screen_width - 105, Y + (19 * offset), g.m_screen_width, 18,
			color_t(0, 0, 0, g_vars.get_as<int>("ui->col->a").value()));

		g_render.draw_filled_rect(g.m_screen_width - 102, Y + 3 + (19 * offset), 2, 12,
			color_t(164, 164, 164));
		...
		g_render.draw_stringW(player_name, g.m_screen_width - 96, Y + 2 + (19 * offset),
			g_render.get_font(Tahoma12px), TEXT_OUTLINE, color_t(255, 255, 255));
...
void visuals::draw_status_bomb_info(c_base_plantedc4* entity, const float explode_time)
{
	...
	g_render.draw_filled_rect(g.m_screen_width - 175 - string_width, 10, string_width + 12, 17,
		color_t(0, 0, 0, g_vars.get_as<int>("ui->col->a").value()));

	g_render.draw_filled_rect(g.m_screen_width - 172 - string_width, 13, 2, 11, color_t("ui->col"));

	g_render.draw_string(bomb_info, g.m_screen_width - 167 - string_width, 12,
		g_render.get_font(Tahoma12px), TEXT_OUTLINE, color_t(255, 255, 255));
}
