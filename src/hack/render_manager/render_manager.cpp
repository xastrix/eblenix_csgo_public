...
bool render_manager::create_objects()
{
	...
	g.m_screen_width = vp.Width;
	g.m_screen_height = vp.Height;
...
void render_manager::begin_render_states()
{
	...
	D3DVIEWPORT9 vp{ 0, 0, g.m_screen_width, g.m_screen_height, 0.0f, 1.0f };
...
void render_manager::draw_status(const std::string& string, color_t color)
{
	...
	draw_filled_rect(g.m_screen_width - string_width - 15, 10, g.m_screen_width, 17, color_t(0, 0, 0, 100));
	draw_filled_rect(g.m_screen_width - string_width - 12, 13, 2, 11, color);

	draw_string(string, g.m_screen_width - string_width - 5, 12,
