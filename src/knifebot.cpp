...
void knifebot::run(i_user_cmd* cmd)
{
	...
	if (!g_vars.get_as<bool>("knifebot->flash_check").value() && g_csgo.get_local()->is_flashed())
		return;
	...
	for (int i = 1; i <= g_csgo.m_globals->max_clients; i++)
	{
		...
		if (!g_vars.get_as<bool>("knifebot->smoke_check").value() && Helpers::is_behind_smoke(g_csgo.get_local()->get_eye_pos(), entity->get_hitbox_position(hitbox_head)))
			return;

		if (!g_vars.get_as<bool>("knifebot->teammate_check").value() && g_csgo.get_local()->get_team_num() == entity->get_team_num())
			return;
