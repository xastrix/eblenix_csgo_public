...
{
	if (g::initialized && !g::panic_mode)
	{
		/* This is necessary so that when using the open console or chat window,
		   the menu cannot be used.
		*/
		g::console_visible = g_csgo.m_engine->is_console_visible();
		g::chat_visible = g_csgo.m_client->is_chat_raised();
...
{
	std::call_once(g::init_render, [device]() {
