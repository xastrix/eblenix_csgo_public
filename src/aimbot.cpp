...
void c_aimbot::run(user_cmd_t* cmd)
{
	...
	if (const auto target = Helpers::find_target_entity(cmd, m_aim_fov, m_angle))
	{
		...
		set_aim_angle(cmd, entity, g_cs->get_local()->aim_punch_angle());
...
{
	if (!g_var->get_as<bool>(V_KNIFEBOT_ENABLED).value())
		return;
	...
	if (!g_var->get_as<bool>(V_KNIFEBOT_FLASH_CHECK).value() && g_cs->get_local()->is_flashed())
		return;
	...
	for (int i = 1; i <= g_cs->m_globals->max_clients; i++)
	{
		...
		if (!g_var->get_as<bool>(V_KNIFEBOT_SMOKE_CHECK).value() && Helpers::is_behind_smoke(g_cs->get_local()->get_eye_pos(), entity->get_hitbox_position(hitbox_head)))
			return;

		if (!g_var->get_as<bool>(V_KNIFEBOT_TEAMMATE_CHECK).value() && g_cs->get_local()->get_team_num() == entity->get_team_num())
			return;

		const auto distance = g_cs->get_local()->get_eye_pos().distance_to(entity->get_eye_pos());
		...
		{
			if (distance <= g_var->get_as<float>(V_KNIFEBOT_DISTANCE).value())
			{
...
void c_aimbot::set_weapon_param(c_base_weapon* weapon)
{
	if (Helpers::is_pistol(weapon))
	{
		...
		m_aim_fov = g_var->get_as<float>(V_AIMBOT_FOV_PISTOL).value();
		m_aim_smooth = g_var->get_as<float>(V_AIMBOT_SMOOTH_PISTOL).value();
		m_aim_type = g_var->get_as<int>(V_AIMBOT_TYPE_PISTOL).value();
	}
	else if (Helpers::is_rifle(weapon))
	{
		...
		m_aim_fov = g_var->get_as<float>(V_AIMBOT_FOV_RIFLE).value();
		m_aim_smooth = g_var->get_as<float>(V_AIMBOT_SMOOTH_RIFLE).value();
		m_aim_type = g_var->get_as<int>(V_AIMBOT_TYPE_RIFLE).value();
	}
	else if (Helpers::is_sniper(weapon))
	{
		...
		m_aim_fov = g_var->get_as<float>(V_AIMBOT_FOV_SNIPER).value();
		m_aim_smooth = g_var->get_as<float>(V_AIMBOT_SMOOTH_SNIPER).value();
		m_aim_type = g_var->get_as<int>(V_AIMBOT_TYPE_SNIPER).value();
	}
	else if (Helpers::is_heavy(weapon))
	{
		...
		m_aim_fov = g_var->get_as<float>(V_AIMBOT_FOV_HEAVY).value();
		m_aim_smooth = g_var->get_as<float>(V_AIMBOT_SMOOTH_HEAVY).value();
		m_aim_type = g_var->get_as<int>(V_AIMBOT_TYPE_HEAVY).value();
	}
	else if (Helpers::is_smg(weapon))
	{
		...
		m_aim_fov = g_var->get_as<float>(V_AIMBOT_FOV_SMG).value();
		m_aim_smooth = g_var->get_as<float>(V_AIMBOT_SMOOTH_SMG).value();
		m_aim_type = g_var->get_as<int>(V_AIMBOT_TYPE_SMG).value();
	}
}

bool c_aimbot::can_aiming(c_base_player* entity, c_base_weapon* weapon)
{
	if (g_var->get_as<bool>(V_AIMBOT_VISIBLE_CHECK).value() && !g_cs->get_local()->can_see_entity(entity->get_eye_pos()))
		return false;
...
void c_aimbot::set_aim_angle(user_cmd_t* cmd, c_base_player* entity, const vec3& aim_punch)
{
	switch (m_aim_type) {
	case 0: {
		m_angle = Math::calculate_angle(g_cs->get_local()->get_eye_pos(),
