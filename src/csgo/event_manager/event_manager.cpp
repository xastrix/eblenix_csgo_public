...
#include "../../hack/var_manager/var_manager.h"
#include "../../hack/helpers/helpers.h"
#include "../../hack/render_manager/render_manager.h"
#include "../../hack/features/esp/esp.h"
...
void event_manager::fire_game_event(c_game_event* event)
{
	...
	case fnv::hash("player_hurt"): {
		if (g_vars.get_as<bool>("misc->event_logs->player_hurt").value()) {
	...
	case fnv::hash("item_purchase"): {
		if (g_vars.get_as<bool>("misc->event_logs->player_purchase").value()) {
			auto ent_index = g_csgo.m_engine->get_player_for_user_id(event->get_int("userid"));
