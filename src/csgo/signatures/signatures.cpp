...
void sig::init()
{
	...
	s_glow_manager            = scan_sig(g.module_list[clientDLL], "0F 11 05 ? ? ? ? 83 C8 01 C7 05 ? ? ? ? 00 00 00 00") + 3;
	s_weapon_system           = scan_sig(g.module_list[clientDLL], "8B 35 ? ? ? ? FF 10 0F B7 C0") + 2;
	s_view_matrix             = scan_sig(g.module_list[clientDLL], "0F 10 05 ? ? ? ? 8D 85 ? ? ? ? B9");
	s_line_goes_through_smoke = scan_sig(g.module_list[clientDLL], "55 8B EC 83 EC 08 8B 15 ? ? ? ? 0F 57 C0");
